...
const ShiftAssignmentModal = ({ isOpen, onClose, shift, eventId, onUpdate }) => {
  ...
  const fetchShiftData = async () => {
    try {
      ...
      const registrationsRes = await fetch(`/api/shifts/${shift.id}/registrations`, {
        headers: {
      ...
      const invitationsRes = await fetch(`/api/events/${eventId}/invitations`, {
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`
        }
      });
      ...
      const acceptedStaff = invitationsData
        ...
        .filter(staff => !registrationsData.some(reg => reg.staff_id === staff.staff_id));
  ...
  const handleAssignStaff = async (staffId, action = 'assign') => {
    try {
      const response = await fetch(`/api/shifts/${shift.id}/assign`, {
        method: 'POST',
        headers: {
      ...
      if (!response.ok) throw new Error('Fehler bei der Zuteilung');
      ...
      showMessage('Fehler bei der Zuteilung', 'error');
    }
  };

  const handleBulkAssignment = async () => {
    if (selectedStaff.length === 0) {
      showMessage('Bitte wählen Sie mindestens einen Mitarbeiter aus', 'error');
      return;
    }
    ...
    try {
      ...
      const response = await fetch(`/api/shifts/${shift.id}/bulk-assign`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${localStorage.getItem('token')}`
      ...
      if (!response.ok) throw new Error('Fehler bei der Massenzuteilung');
      
      showMessage(`${selectedStaff.length} Mitarbeiter erfolgreich zugeteilt`);
      setSelectedStaff([]);
      fetchShiftData();
    } catch (error) {
      showMessage('Fehler bei der Massenzuteilung', 'error');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleConfirmAssignment = async (registrationId) => {
    try {
      const response = await fetch(`/api/shifts/registrations/${registrationId}/confirm`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`
        }
      ...
      showMessage('Zuteilung als endgültig markiert');
      fetchShiftData();
    } catch (error) {
      showMessage('Fehler beim Bestätigen', 'error');
    }
  };
